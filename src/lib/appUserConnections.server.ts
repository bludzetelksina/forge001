import { decryptSecret, encryptSecret } from "./connectionKeyCrypto.server";
  ...
  const { error } = await supabaseAdmin.from("app_user_connections").upsert(
    {
      ...
      connection_key_ciphertext: encryptSecret(connectionAPIKey),
...
export async function getConnectionKeyForUser(userId: string, connectorId: string) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) throw error;
  return data ? decryptSecret(data.connection_key_ciphertext) : null;
}
